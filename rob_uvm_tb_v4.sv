//******************************************************************************
// UVM Testbench — BOOM ROB — Revision 2.1 (rob-testplan-full.pdf)
// Config: CW=4, 10 WB ports, 24 rows, 96 entries, 20-bit br_mask
// TC0a-TC0e (smoke), TC1-TC5 (directed), TC6 (random), TC7-TC14 (gap-closing)
//******************************************************************************
`timescale 1ns/1ps

// ==== FILE: rob_if.sv ====
interface rob_if(input logic clock, input logic reset);
  parameter CW=4, WB=10, LSU=2, FF=2;
  // Dispatch
  logic        enq_valids [CW];
...
package rob_pkg;
  ...
  parameter int NUM_LSU_CLR    = 2;
  parameter int NUM_FFLAGS     = 2;
...
// ==== FILE: rob_transaction.sv ====
  ...
  class rob_transaction extends uvm_sequence_item;
    ...
    // FFlags
    rand bit        fflags_valid [2];
    rand bit [6:0]  fflags_rob_idx [2];
    rand bit [4:0]  fflags_flags [2];
    // Load exception
    ...
    function string convert2string();
      return $sformatf("op=%s", op.name());
    endfunction
  endclass

// ==== FILE: rob_shadow_model.sv ====
  typedef struct {
    bit valid, busy, unsafe, exception;
    bit [6:0] pdst;
    bit [19:0] br_mask;
    bit is_fence;
  } shadow_entry_t;
  ...
  class rob_shadow_model extends uvm_object;
    ...
    function bit dispatch(int idx,bit[6:0] pdst,bit xcpt,bit is_fence,bit[19:0] br_mask,bit unsafe_in);
    ...
    function bit writeback(int idx,bit[6:0] pdst);
      ...
      e[idx].busy=0; e[idx].unsafe=0; return 1;
    endfunction

    function bit can_writeback(int idx);
      return (e[idx].valid && e[idx].busy);
    endfunction

    function void lsu_clr(int idx);
      if(e[idx].valid) e[idx].busy=0;
    endfunction
...
// ==== FILE: rob_driver.sv ====
  class rob_driver extends uvm_driver #(rob_transaction);
    ...
    // Hàm cho TC6 random gọi
    function bit can_writeback(bit[6:0] idx);
      return shadow.can_writeback(idx);
    endfunction
    ...
    task clear();
      for(int i=0;i<CW;i++) begin
        ...
        vif.drv_cb.enq_uops_is_fencei[i]<=0; vif.drv_cb.enq_uops_unsafe[i]<=0;
      ...
      for(int i=0;i<CW;i++) begin
        ...
        if(tr.enq_valids[i])
          shadow.dispatch(tr.enq_uops_rob_idx[i], tr.enq_uops_pdst[i],
            tr.enq_uops_exception[i], tr.enq_uops_is_fence[i],
            tr.enq_uops_br_mask[i], tr.enq_uops_unsafe[i]);
      end
    ...
    task check();
      ...
      for(int i=0;i<CW;i++) if(vif.mon_cb.enq_valids[i]) begin disp_cnt++;
        shadow.dispatch(vif.mon_cb.enq_uops_rob_idx[i],vif.mon_cb.enq_uops_pdst[i],
          ...
          vif.mon_cb.enq_uops_br_mask[i],vif.mon_cb.enq_uops_unsafe[i]);
      end
  ...
  class rob_seq_base extends uvm_sequence #(rob_transaction);
    ...
    task dispatch_row(int row, bit [19:0] br_mask=0, bit xcpt_bank=-1,
      ...
      for(int b=0;b<CW;b++) begin
        ...
        tr.enq_uops_unsafe[b] = fence_mask[b] ? 0 : 1;
    ...
    task dispatch_partial(int row, bit[3:0] valid_mask, bit partial=1);
      ...
      for(int b=0;b<CW;b++) begin
        ...
        tr.enq_uops_unsafe[b] = 1; tr.enq_uops_ldst_val[b] = 1;
    ...
    task wait_empty(int timeout=30);
      ...
      if(!vif.empty) `uvm_warning("SEQ","ROB not empty after timeout")
    endtask
...
// ==== FILE: rob_seq_tc0_smoke.sv ====
  ...
  class tc0b_single_uop_seq extends rob_seq_base;
    ...
    task body();
      ...
      wait_empty();
      @(posedge vif.clock);
      if(vif.empty) `uvm_info("TC0b","PASS",UVM_LOW) else `uvm_error("TC0b","FAIL")
    endtask
  endclass
  ...
  class tc0c_full_row_seq extends rob_seq_base;
    ...
    task body();
      ...
      wait_empty();
      @(posedge vif.clock);
      if(vif.empty) `uvm_info("TC0c","PASS",UVM_LOW) else `uvm_error("TC0c","FAIL")
    endtask
  endclass
  ...
  class tc0d_cascading_seq extends rob_seq_base;
    ...
    task body();
      ...
      wait_empty();
      @(posedge vif.clock);
      if(vif.empty) `uvm_info("TC0d","PASS: cascading commit",UVM_LOW) else `uvm_error("TC0d","FAIL")
    endtask
  endclass
  ...
  class tc0e_full_bp_seq extends rob_seq_base;
    ...
    task body();
      ...
      for(int r=0;r<NUM_ROB_ROWS;r++) begin
        if(!vif.ready) begin `uvm_info("TC0e",$sformatf("PASS: full at row %0d",r),UVM_LOW) break; end
        dispatch_row(r);
      end
      @(posedge vif.clock);
      if(!vif.ready) `uvm_info("TC0e","PASS: io_ready=0",UVM_LOW) else `uvm_error("TC0e","FAIL: still ready")
      idle(5);
    endtask
  endclass
...
// ==== FILE: rob_seq_tc1_dispatch.sv ====
  class tc1_dispatch_seq extends rob_seq_base;
    ...
    task body();
      ...
      @(posedge vif.clock);
      `uvm_info("TC1",$sformatf("After partial: tail=%0d",vif.rob_tail_idx),UVM_LOW)

      // Phase B: Full dispatch + ROB full
      `uvm_info("TC1","Phase B: Fill ROB",UVM_LOW)
      for(int r=1;r<NUM_ROB_ROWS;r++) begin
        if(!vif.ready) begin `uvm_info("TC1",$sformatf("F1.4 PASS: full at row %0d",r),UVM_LOW) break; end
        dispatch_row(r);
      end
      ...
      for(int iter=0;iter<6;iter++) begin
        ...
        if(vif.ready) dispatch_row((NUM_ROB_ROWS+iter)%NUM_ROB_ROWS);
      end
...
// ==== FILE: rob_seq_tc2_ooo_writeback.sv ====
  class tc2_ooo_writeback_seq extends rob_seq_base;
    ...
    task body();
      ...
      wb_row(1); // younger
      @(posedge vif.clock);
      if(vif.commit_valids[0]) `uvm_error("TC2","FAIL: premature commit")
      ...
      idle(3);
      @(posedge vif.clock);
      if(!vif.empty) `uvm_info("TC2","PASS: no commit until head WB",UVM_LOW)
      wb_row(0); // oldest
      wait_empty();
      if(vif.empty) `uvm_info("TC2","PASS: cascading commit after head WB",UVM_LOW)
...
// ==== FILE: rob_seq_tc3_inorder_commit.sv ====
  class tc3_inorder_commit_seq extends rob_seq_base;
    ...
    task body();
      ...
      wb_row(0); wb_row(1);
      wait_empty();
      if(vif.empty) `uvm_info("TC3","Phase A PASS",UVM_LOW)
      ...
      wait_empty();
      if(vif.empty) `uvm_info("TC3","Phase B PASS: drain to empty",UVM_LOW)
      ...
      wait_empty();
      if(vif.empty) `uvm_info("TC3","Phase C PASS",UVM_LOW)
  ...
  class tc4_precise_exception_seq extends rob_seq_base;
    ...
    task body();
      ...
      repeat(5) begin idle(1); @(posedge vif.clock);
        if(vif.com_xcpt_valid) `uvm_info("TC4","F4.3 PASS: com_xcpt_valid pulse",UVM_LOW)
        if(vif.flush_valid) `uvm_info("TC4",$sformatf("F4.4 PASS: flush typ=%0d",vif.flush_bits_flush_typ),UVM_LOW)
      end
      wait_recovery();
      if(vif.empty && vif.ready) `uvm_info("TC4","F4.11 PASS: post-flush recovery",UVM_LOW)
      ...
      idle(5);
      @(posedge vif.clock);
      if(!vif.com_xcpt_valid) `uvm_info("TC4","F4.2 PASS: no throw while busy",UVM_LOW)
      else `uvm_error("TC4","F4.2 FAIL: early exception")
      wb_row(0);
      repeat(3) begin idle(1); @(posedge vif.clock);
        if(vif.com_xcpt_valid) `uvm_info("TC4","PASS: throw after WB",UVM_LOW)
      end
      wait_recovery();
      ...
      repeat(5) begin idle(1); @(posedge vif.clock);
        if(vif.com_xcpt_valid) begin
          ...
          `uvm_info("TC4",$sformatf("F4.10: flush_frontend=%0b",vif.flush_frontend),UVM_LOW)
        end
      end
      wait_recovery();
      ...
      begin
        ...
        idle(5);
        @(posedge vif.clock);
        if(!vif.com_xcpt_valid) `uvm_info("TC4","PASS: stall blocks exception",UVM_LOW)
        dvif.drv_cb.csr_stall <= 0;
        repeat(5) begin idle(1); @(posedge vif.clock);
          if(vif.com_xcpt_valid) `uvm_info("TC4","PASS: xcpt after stall release",UVM_LOW)
        end
        wait_recovery();
      end
      ...
      dispatch_row(0, .xcpt_bank(0)); idle(1); wb_row(0);
      wait_recovery();
      if(vif.empty) `uvm_info("TC4","PASS: no residual state",UVM_LOW)
...
// ==== FILE: rob_seq_tc5_branch_mispredict.sv ====
  class tc5_branch_mispredict_seq extends rob_seq_base;
    ...
    task body();
      ...
      repeat(3) begin idle(1); @(posedge vif.clock);
        `uvm_info("TC5",$sformatf("tail=%0d head=%0d rbk=%0b",vif.rob_tail_idx,vif.rob_head_idx,vif.commit_rollback),UVM_MEDIUM)
      end
      @(posedge vif.clock);
      if(vif.rob_tail_idx < tail_before) `uvm_info("TC5","F5.2 PASS: tail snapped back",UVM_LOW)
      wait_recovery();
  ...
  class tc6_random_seq extends uvm_sequence #(rob_transaction);
    ...
    task do_random_dispatch();
      ...
      for(int b=0;b<4;b++) begin
        ...
        tr.enq_uops_unsafe[b]=1; tr.enq_uops_uses_ldq[b]=0; tr.enq_uops_uses_stq[b]=0;
    ...
    task do_random_mispredict();
      ...
      begin
        ...
        tr.br_rob_idx=br_idx; tr.br_mispredict=1; tr.br_taken=$urandom_range(0,1);
        finish_item(tr); @(posedge vif.clock);
...
// ==== FILE: rob_seq_tc7_fence.sv ====
  class tc7_fence_seq extends rob_seq_base;
    ...
    task body();
      ...
      wait_empty();
      if(vif.empty) `uvm_info("TC7","Phase A PASS",UVM_LOW)
      ...
      wait_empty();
      if(vif.empty) `uvm_info("TC7","Phase B PASS",UVM_LOW)
      // Phase C: all-fence row
      dispatch_row(0, .fence_mask(4'b1111));
      // No WB needed — all busy=0
      wait_empty();
      if(vif.empty) `uvm_info("TC7","Phase C PASS: all-fence commit",UVM_LOW)
...
// ==== FILE: rob_seq_tc8_lsu_clr.sv ====
  class tc8_lsu_clr_seq extends rob_seq_base;
    ...
    task body();
      ...
      lsu_clr(`ROB_IDX(0,3));
      wait_empty();
      if(vif.empty) `uvm_info("TC8","Phase A PASS",UVM_LOW)
      ...
      wait_empty();
      if(vif.empty) `uvm_info("TC8","Phase B PASS: simultaneous",UVM_LOW)
...
// ==== FILE: rob_seq_tc9_csr_stall.sv ====
  class tc9_csr_stall_seq extends rob_seq_base;
    ...
    task body();
      ...
      idle(5);
      @(posedge vif.clock);
      if(!(vif.commit_valids[0]||vif.commit_valids[1]||vif.commit_valids[2]||vif.commit_valids[3]))
      ...
      @(posedge vif.clock);
      if(!vif.com_xcpt_valid) `uvm_info("TC9","Phase C PASS: xcpt blocked by stall",UVM_LOW)
      dvif.drv_cb.csr_stall<=0;
      wait_recovery();
      ...
      repeat(5) begin idle(1); @(posedge vif.clock);
        if(vif.commit_valids[0]) begin
          `uvm_info("TC10",$sformatf("arch_valids={%0b,%0b,%0b,%0b}",
            vif.commit_arch_valids[0],vif.commit_arch_valids[1],
            vif.commit_arch_valids[2],vif.commit_arch_valids[3]),UVM_LOW)
    ...
    task body();
      ...
      begin
        ...
        for(int b=0;b<4;b++) begin
          ...
          tr.enq_uops_unsafe[b]=0; tr.enq_uops_ldst_val[b]=1;
        end
        finish_item(tr);
      end
      idle(1); wb_row(0);
      repeat(8) begin idle(1); @(posedge vif.clock);
        if(vif.com_xcpt_valid) begin
          if(vif.com_xcpt_bits_cause==64'd2)
            ...
            `uvm_error("TC11",$sformatf("FAIL: cause=%0d (expected 2)",vif.com_xcpt_bits_cause))
          break;
        end
      end
      wait_recovery();
...
// ==== FILE: rob_seq_tc13_fsm_wait.sv ====
  class tc13_fsm_wait_seq extends rob_seq_base;
    ...
    task body();
      ...
      @(posedge vif.clock);
      if(!vif.ready) `uvm_info("TC13","F7.4 PASS: io_ready=0 after is_unique",UVM_LOW)
      idle(1); wb_row(1);
      wait_empty();
      @(posedge vif.clock);
      if(vif.ready) `uvm_info("TC13","F7.5 PASS: io_ready=1 after drain",UVM_LOW)

      // Phase B: is_unique with many entries
      for(int r=0;r<5;r++) dispatch_row(r);
      dispatch_row(5, .unique_mask(4'b0001));
      @(posedge vif.clock);
      if(!vif.ready) `uvm_info("TC13","Phase B: wait state entered",UVM_LOW)
      for(int r=0;r<=5;r++) begin idle(1); wb_row(r); end
      wait_empty();
      ...
      idle(1); wb_row(0);
      wait_empty();
      if(vif.empty) `uvm_info("TC13","Phase D PASS: fast path",UVM_LOW)
...
// ==== FILE: rob_seq_tc14_fflags.sv ====
  class tc14_fflags_seq extends rob_seq_base;
    ...
    task body();
      ...
      dvif.drv_cb.fflags_valid[1]<=1;
      dvif.drv_cb.fflags_uop_rob_idx[1]<=`ROB_IDX(0,2);
      dvif.drv_cb.fflags_bits_flags[1]<=5'b10000;
      idle(1);
      dvif.drv_cb.fflags_valid[0]<=0; dvif.drv_cb.fflags_valid[1]<=0;
      repeat(5) begin idle(1); @(posedge vif.clock);
        if(vif.commit_fflags_valid)
          `uvm_info("TC14",$sformatf("fflags=%05b",vif.commit_fflags_bits),UVM_LOW)
      end
      wait_empty();
      ...
      repeat(5) begin idle(1); @(posedge vif.clock);
        if(vif.flush_valid)
          `uvm_info("TC14",$sformatf("PASS: flush_on_commit typ=%0d",vif.flush_bits_flush_typ),UVM_LOW)
      end
      wait_recovery();
...
module tb_top;
  ...
  // -------------------------------------------------------
  // DUT instantiation — CW=4, 10 WB ports
  // Port names follow Chisel-generated pattern: io_enq_uops_<slot>_<field>
  // Adjust port names to match your actual Rob.v
  // -------------------------------------------------------
  Rob dut (
    ...
    // Enqueue uops slot 0
    .io_enq_uops_0_uopc(rif.enq_uops_uopc[0]), .io_enq_uops_0_is_rvc(rif.enq_uops_is_rvc[0]),
    ...
    // Slot 1
    .io_enq_uops_1_uopc(rif.enq_uops_uopc[1]), .io_enq_uops_1_is_rvc(rif.enq_uops_is_rvc[1]),
    ...
    // Slot 2
    .io_enq_uops_2_uopc(rif.enq_uops_uopc[2]), .io_enq_uops_2_is_rvc(rif.enq_uops_is_rvc[2]),
    ...
    // Slot 3
    .io_enq_uops_3_uopc(rif.enq_uops_uopc[3]), .io_enq_uops_3_is_rvc(rif.enq_uops_is_rvc[3]),
    .io_enq_uops_3_debug_inst(rif.enq_uops_debug_inst[3]),
    ...
    .io_wb_resps_6_bits_uop_pdst(rif.wb_resps_pdst[6]),
    ...
    .io_fflags_1_valid(rif.fflags_valid[1]), .io_fflags_1_bits_uop_rob_idx(rif.fflags_uop_rob_idx[1]),
    ...
    .io_commit_uops_0_ldst_val(rif.commit_uops_ldst_val[0]), .io_commit_uops_0_ldst(rif.commit_uops_ldst[0]),
    .io_commit_uops_1_pdst(rif.commit_uops_pdst[1]), .io_commit_uops_1_stale_pdst(rif.commit_uops_stale_pdst[1]),
    .io_commit_uops_1_ldst_val(rif.commit_uops_ldst_val[1]), .io_commit_uops_1_ldst(rif.commit_uops_ldst[1]),
    .io_commit_uops_2_pdst(rif.commit_uops_pdst[2]), .io_commit_uops_2_stale_pdst(rif.commit_uops_stale_pdst[2]),
    .io_commit_uops_2_ldst_val(rif.commit_uops_ldst_val[2]), .io_commit_uops_2_ldst(rif.commit_uops_ldst[2]),
    .io_commit_uops_3_pdst(rif.commit_uops_pdst[3]), .io_commit_uops_3_stale_pdst(rif.commit_uops_stale_pdst[3]),
    .io_commit_uops_3_ldst_val(rif.commit_uops_ldst_val[3]), .io_commit_uops_3_ldst(rif.commit_uops_ldst[3]),
    ...
    .io_com_xcpt_bits_cause(rif.com_xcpt_bits_cause), .io_com_xcpt_bits_badvaddr(rif.com_xcpt_bits_badvaddr),
    .io_flush_valid(rif.flush_valid), .io_flush_bits_flush_typ(rif.flush_bits_flush_typ),
